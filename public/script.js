...
function renderSessionsList() {
  ...
  sessions.forEach(session => {
    ...
    li.onclick = () => switchToSession(session.sessionId);
...
async function connectWebSocket() {
  ...
  const url = `${protocol}//${location.hostname}${port}/api/ws?session_id=${currentSessionId}`;
...
function updateConnectionStatus(txt, cls) {
  ...
  if (statusText) {
    ...
    statusText.className = cls === 'bg-teal-500' ? 'text-teal-400' : 
                           cls === 'bg-red-500' ? 'text-red-400' : 'text-gray-400';
  }
}
...
window.useSuggestion = function(el) {
  const txt = el.textContent.trim().replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, '').replace(/[^\w\s\?]/g, '').trim();
  userInput.value = txt;
  userInput.style.height = 'auto';
  userInput.style.height = Math.min(userInput.scrollHeight, 200) + 'px';
  userInput.focus();
